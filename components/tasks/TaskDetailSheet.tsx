  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && dispatch(closeTaskDetail())}>
      <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto p-0">
        {/* Header Bar */}
        <div className="p-5 border-b border-slate-800/80 bg-slate-900/30 sticky top-0 z-10 backdrop-blur-md">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-2 text-xs font-mono text-slate-400">
              ...
              <span>•</span>
              <span className="text-slate-400">{project?.name || 'General'}</span>
            </div>
            ...
            <div className="flex items-center space-x-1.5 mr-6">
              {userCanEdit && (
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 text-xs text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-950/30"
              ...
              {userCanEdit && (
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 text-xs text-slate-700 dark:text-slate-300"
          ...
          {isEditingTitle ? (
            <Input
              ...
              autoFocus
              className="text-lg font-semibold bg-white dark:bg-slate-900 border-purple-500"
            />
          ) : (
            <h2
              ...
              className={`text-lg font-semibold text-slate-900 dark:text-slate-100 leading-tight ${
                userCanEdit ? 'cursor-pointer hover:text-purple-600 dark:hover:text-purple-300' : ''
              }`}
        ...
        <div className="p-6 space-y-6">
          {/* Metadata Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40">
            {/* Status */}
            <div>
                ...
                const renderItem = (sub: Subtask, level = 0) => {
                  ...
                  return (
                    <div key={sub.id} className="space-y-1.5">
                      <div
                        className={`group flex items-center justify-between p-2 rounded-lg border border-slate-200 dark:border-slate-800/80 bg-slate-50/70 dark:bg-slate-900/40 hover:bg-slate-100 dark:hover:bg-slate-850 transition-all ${
                          level > 0 ? 'ml-5 pl-2.5 border-l-2 border-l-purple-500/50' : ''
                        }`}
                      >
                        <div className="flex items-center space-x-2.5 flex-1 min-w-0 mr-2">
                          {level > 0 && <CornerDownRight className="w-3 h-3 text-purple-500 shrink-0 opacity-70" />}
                          <Checkbox
                          ...
                          {editingSubtaskId === sub.id ? (
                            <input
                              ...
                              autoFocus
                              className="text-xs bg-white dark:bg-slate-950 px-2 py-0.5 rounded border border-purple-500 text-slate-800 dark:text-slate-200 w-full"
                            />
                          ) : (
                            <span
                              onClick={() => {
                        ...
                        <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {userCanEdit && level < 3 && (
                            <button
                              ...
                              className="p-1 rounded text-slate-500 dark:text-slate-400 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-purple-50 dark:hover:bg-slate-800"
                          ...
                          {userCanEdit && (
                            <button
                              ...
                              className="p-1 rounded text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-slate-800"
                          ...
                          {userCanEdit && (
                            <button
                              ...
                              className="p-1 rounded text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-slate-800"
                      ...
                      {isAddingChild && (
                        <div className={`flex items-center space-x-2 pl-6 py-1 ${level > 0 ? 'ml-5' : ''}`}>
                          <Input
                            ...
                            className="h-7 text-xs bg-white dark:bg-slate-900 border-purple-300 dark:border-purple-800"
                            autoFocus
                          />
                          <Button
                            size="sm"
                ...
                <div className="space-y-2">
                  {activities.map((act) => {
                    ...
                    return (
                      <div
                        key={act.id}
                        className="flex items-start space-x-2.5 p-2 rounded-lg border border-slate-850 bg-slate-900/30 text-xs"
                      >
                        ...
                        <div className="flex-1">
                          ...
                          {act.details && <div className="text-slate-400 text-[11px] mt-0.5">{act.details}</div>}
